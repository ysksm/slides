...
スライドは **カテゴリ別** に並べています。[公開サイト](https://ysksm.github.io/slides/)の目次ページでは、カテゴリのボタンで絞り込み、題名・概要・タグ・Issue 番号でスライドを検索できます（`?q=検索語&cat=カテゴリ id` の URL で共有可能）。

## カテゴリ

| カテゴリ | id | 内容 |
|---|---|---|
| [AI エージェント・Claude Code](#ai-エージェントclaude-code) | `ai-agent` | Claude Code・Codex などのコーディングエージェントの使い方と費用、オーケストレーション、AI を使った自動実装の検討 |
| [テスト・開発ツール](#テスト開発ツール) | `testing-tooling` | Playwright や Lint など、テストと開発ツールの解説・活用方法 |
| [言語・言語サーバー](#言語言語サーバー) | `language` | Go・TypeScript など言語そのもののチュートリアルと、言語サーバーの調査 |
| [組み込み Linux](#組み込み-linux) | `embedded` | Yocto によるイメージ作成とビルド環境の構築 |
| [Web アプリ設計](#web-アプリ設計) | `web-app` | 既存システムの UI 置き換えや配信構成の技術検討 |
| [セキュリティ・依存関係](#セキュリティ依存関係) | `security` | ライブラリの脆弱性・不具合の集計と可視化 |

カテゴリの定義（id・表示名・説明・並び順）は [`scripts/categories.tsv`](scripts/categories.tsv) にあります。各カテゴリ内は新しい順です。
...
| スライド | 枚数 | 形式 | 関連 Issue |
|---|---:|---|---|
| [Go 言語チュートリアル ハンズオン ① 基礎編 — Hello, World からモジュール・テスト・ワークスペースまで](go-tutorial-basics/index.html) | 20 | 手書き HTML | [#45](https://github.com/ysksm/slides/issues/45) |
| [Go 言語チュートリアル ハンズオン ② 応用編 — データベース・REST API・ジェネリクス・ファジング・govulncheck](go-tutorial-advanced/index.html) | 19 | 手書き HTML | [#45](https://github.com/ysksm/slides/issues/45) |
...
[go.dev の公式チュートリアル](https://go.dev/doc/tutorial/)のうち前半 4 本（Getting started / Create a Go module（7 パート）/ Multi-module workspaces / Working with JSON）を、各ステップの「やってみよう」に沿って手を動かしながら進めるハンズオン形式の解説スライド（全 20 枚）。コマンド・コード・実行結果はすべて Go 1.27.1 / 1.24.7 で実際に実行して確認したものを載せている。続編は [② 応用編](go-tutorial-advanced/index.html)。
...
[go.dev の公式チュートリアル](https://go.dev/doc/tutorial/)のうち後半 6 本（Accessing a relational database / RESTful API with Go and Gin / Generics / Fuzzing / govulncheck / govulncheck with VS Code Go）を、各ステップの「やってみよう」に沿って手を動かしながら進めるハンズオン形式の解説スライド（全 19 枚）。テーマごとに独立しているので、必要なところから始められる。データベースの章以外は実際に実行して出力を確認している。前編は [① 基礎編](go-tutorial-basics/index.html)。
...
- `antigravity-tsc-lsp-research/index.html` を開き、← → / Space で移動、O で一覧、N でノート、F で全画面
...
[Yocto 入門](yocto-intro/index.html)の付録として、ビルドホストに使う Linux ディストリビューションの選び方と、インストールから最初のビルドが通るまでの手順だけを単独で扱う解説スライド（全 18 枚）。サポート対象ディストロの選定基準、ベアメタル / VM / WSL 2 / コンテナの選択、ディスクとファイルシステムの設計、インストール直後の初期設定、ホスト依存パッケージ、`buildtools-tarball` による救済、`core-image-minimal` での動作確認、運用とチューニングまでを通す。
...
```
slides/
├── go-tutorial-basics/            # Go 言語チュートリアル ハンズオン ① 基礎編
├── go-tutorial-advanced/          # Go 言語チュートリアル ハンズオン ② 応用編
├── yocto-build-host/              # Yocto ビルドホストの構築手順（ディストロ選択とインストール）
...
├── scripts/
│   ├── categories.tsv             #   目次のカテゴリ定義（id・表示名・説明・並び順）
│   └── check-toc.sh               #   目次とデッキの整合チェック（未登録・枚数・カテゴリ・リンク）
├── index.html                     # GitHub Pages のトップページ（カテゴリ別の目次・検索）
├── .nojekyll                      # Pages で Jekyll 処理を無効化
└── .github/workflows/             # GitHub Pages デプロイ・目次分離チェックのワークフロー
```
...
| 全画面 | `F` | すべて |
| 印刷 / PDF | `P` | Marp 製、`antigravity-tsc-lsp-research`、`tsc-lsp` 以外のすべて |

※ Marp 製の `claude-code-commands` では `P` はプレゼンターモードです。`antigravity-tsc-lsp-research` と `tsc-lsp`（初版）は `P` に未対応のため、ブラウザの印刷機能を使ってください。

## ビルド

Marp 製の `claude-code-commands` 以外はすべて手書き HTML で、ビルド不要です。HTML を直接編集します。
...
`main` への push で `.github/workflows/deploy-pages.yml` が動き、リポジトリルートがそのまま GitHub Pages に公開されます。ルートの `index.html` が全スライドの目次で、カテゴリ絞り込みと検索ができます。各スライドは `https://ysksm.github.io/slides/<ディレクトリ名>/` で開けます。

`.nojekyll` を置いて Jekyll による変換を無効化しているため、HTML・CSS・JS はリポジトリ内のファイルがそのまま配信されます。

新しいスライドの目次への登録（この `README.md` と `index.html`）は、デッキの PR とは分けて目次更新 Issue で行います（[CONTRIBUTING.md](CONTRIBUTING.md)）。登録時はカテゴリを `scripts/categories.tsv` から選び、`scripts/check-toc.sh` で整合を確認します。
