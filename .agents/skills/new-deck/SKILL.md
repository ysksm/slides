...
name: new-deck
description: >-
  このリポジトリに新しいスライドデッキ（デッキディレクトリ配下の index.html + README.md）を作る手順。「スライドを作って」「デッキを追加」「〜の解説スライド」「Issue #N のスライド」と言われたときに使う。目次（ルート README.md / index.html）は更新しない。
---
...
- 目次はカテゴリ分け + キーワード検索の構成なので、登録依頼のコメントには **カテゴリ案**（`ai` / `codegen` / `testing` / `devtools` / `web` / `embedded` / `security` のいずれか、または新設案）と **検索キーワード案**（題名・概要に出てこない英名・略語・言い換え）を添える。カテゴリの一覧はルート `README.md` の「スライド一覧」冒頭の表にある。
