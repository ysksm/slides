...
- そのコメントには、目次のカテゴリ候補（`scripts/categories.tsv` の id を 1 つ）と検索用タグの候補（製品名・技術名など数語）を添える。目次はカテゴリ別で検索できる作りなので、登録する側の判断材料になる。`scripts/categories.tsv` 自体はデッキ作業では編集しない。
