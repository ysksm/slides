...
- [実験スクリプト](lab/): スライド内の実験（`loop.py` / `ctx.py` / `mmap_demand.py` / `fork_cow.py` / `pagecache.sh`）を再現するためのもの。
...
| 10 | **メモリ不足**: 回収（ファイルページと匿名ページの LRU、`swappiness`）→ OOM killer、`free` の available の読み方 |
...
出力はすべて本デッキの作成環境（Linux 6.18、x86_64、4 論理 CPU、15 GiB、KVM ゲスト、virtio ディスク、ext4）で 2026-09-16 に実際に得たものです。数値は環境で変わります。作成環境で実行できなかった操作（cgroup の作成、`unshare`）は手順と期待される結果を示しています。スクリプトは [`lab/`](lab/) に同梱しています。
